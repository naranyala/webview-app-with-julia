...
export function parseListDirectoryResult(value) {
  if (!value || typeof value !== 'object' || !Array.isArray(value.entries)) {
    return null;
  }
  return {
    path: cleanString(value.path).slice(0, MAX_PATH),
    entries: value.entries
      .filter((e) => e && typeof e === 'object')
      .map((e) => ({
        name: cleanString(e.name).slice(0, 255),
        path: cleanString(e.path).slice(0, MAX_PATH),
        isDir: Boolean(e.isDir),
        size: boundedNumber(e.size),
        modified: cleanString(e.modified)
      })),
    count: boundedNumber(value.count),
