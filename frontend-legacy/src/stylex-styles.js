import * as stylex from '@stylexjs/stylex';
import { contentStyles } from './stylex-content.js';
import { foundationStyles } from './stylex-foundation.js';
import { mediaStyles } from './stylex-media.js';
import { taskStyles } from './stylex-tasks.js';

export const styles = {
  ...foundationStyles,
  ...contentStyles,
  ...mediaStyles,
  ...taskStyles
};

const aliases = {
  shell: 'shell',
  topbar: 'topbar',
  'workspace-topbar': 'workspaceTopbar',
  'brand-mark': 'brandMark',
  'brand-name': 'brandName',
  'topbar-status': 'topbarStatus',
  'status-dot': 'statusDot',
  'back-button': 'backButton',
  'back-label': 'backLabel',
  'titlebar-name': 'titlebarName',
  'titlebar-dot': 'titlebarDot',
  'window-actions': 'windowActions',
  'close-button': 'closeButton',
  rail: 'rail',
  tab: 'tab',
  'tab-glyph': 'tabGlyph',
  'tab-label': 'tabLabel',
  'tab-dot': 'tabDot',
  'tools-panel': 'sidePanel',
  'tools-group-label': 'groupLabel',
  'tools-item': 'sideItem',
  'tools-item-glyph': 'sideGlyph',
  'tools-item-copy': 'sideCopy',
  'map-page': 'mapPage',
  'map-layout': 'mapLayout',
  'map-panel': 'mapPanel',
  'map-panel-heading': 'mapPanelHeading',
  'map-panel-actions': 'mapPanelActions',
  'map-reset': 'mapReset',
  'map-count': 'mapCount',
  'map-toolbar': 'mapToolbar',
  'map-mode': 'mapMode',
  'map-mode-button': 'mapModeButton',
  'map-select-wrap': 'mapSelectWrap',
  'map-select': 'mapSelect',
  'map-canvas': 'mapCanvas',
  'map-leaflet': 'mapLeaflet',
  'map-overlay': 'mapOverlay',
  'map-overlay-kicker': 'mapOverlayKicker',
  'map-footer': 'mapFooter',
  'map-source-mark': 'mapSourceMark',
  'map-browser': 'mapBrowser',
  'map-browser-heading': 'mapBrowserHeading',
  'map-search-wrap': 'mapSearchWrap',
  'map-search': 'mapSearch',
  'map-list': 'mapList',
  'map-list-item': 'mapListItem',
  'map-list-color': 'mapListColor',
  'map-list-copy': 'mapListCopy',
  'map-list-chevron': 'mapListChevron',
  'map-empty': 'mapEmpty',
  'map-data-panel': 'mapDataPanel',
  'map-data-heading': 'mapDataHeading',
  'map-table-scroll': 'mapTableScroll',
  'map-table': 'mapTable',
  'map-table-head': 'mapTableHead',
  'map-table-cell': 'mapTableCell',
  'map-table-detail': 'mapTableDetail',
  'map-table-row': 'mapTableRow',
  'map-table-action': 'mapTableAction',
  'map-table-action-name': 'mapTableActionName',
  'map-table-empty': 'mapTableEmpty',
  'map-selection': 'mapSelection',
  'map-selection-type': 'mapSelectionType',
  'map-selection-name': 'mapSelectionName',
  'map-source': 'mapSource',
  'launcher-main': 'launcher',
  'launcher-head': 'toolHeading',
  'tool-list': 'toolList',
  'tool-row': 'toolRow',
  'row-glyph': 'rowGlyph',
  'row-copy': 'rowCopy',
  'row-chevron': 'rowChevron',
  'launcher-status': 'launcherStatus',
  'recent-strip': 'recent',
  chip: 'chip',
  error: 'error',
  'workspace-error': 'workspaceError',
  'workspace-body': 'workspaceBody',
  'tool-page': 'toolPage',
  eyebrow: 'eyebrow',
  lede: 'lede',
  'error-title': 'errorTitle',
  'tool-heading': 'toolHeading',
  'mock-badge': 'badge',
  'tool-panel': 'panel',
  'panel-heading': 'panelHeading',
  'panel-label': 'panelLabel',
  'panel-status': 'panelStatus',
  'select-label': 'selectLabel',
  select: 'select',
  'panel-note': 'note',
  'disk-grid': 'diskGrid',
  'volume-summary': 'summaryRow',
  'progress-track': 'bar',
  'primary-button': 'primary',
  'folder-list': 'folderList',
  'folder-copy': 'folderCopy',
  'storage-footer': 'footerRow',
  'notes-layout': 'notesLayout',
  'notes-list-panel': 'notesPanel',
  'note-editor': 'editor',
  'notes-list-heading': 'headingRow',
  'new-note-button': 'action',
  'export-button': 'action',
  'search-field': 'searchInput',
  'search-engine-note': 'searchEngineNote',
  'notes-list': 'notesList',
  'note-list-item': 'noteItem',
  'note-list-meta': 'noteMeta',
  'note-list-updated': 'noteUpdated',
  'empty-notes': 'empty',
  'note-editor-heading': 'headingRow',
  'note-saved': 'saved',
  'note-title-input': 'titleInput',
  'note-meta-row': 'metaRow',
  'note-body-input': 'bodyInput',
  'qna-field': 'qnaField',
  'qna-label': 'qnaLabel',
  'qna-input': 'qnaInput',
  'qna-question-input': 'qnaQuestionInput',
  'qna-answer-input': 'qnaAnswerInput',
  'qna-import': 'qnaImport',
  'qna-help': 'qnaHelp',
  'qna-import-input': 'qnaImportInput',
  'note-editor-footer': 'editorFooter',
  'text-button': 'textButton',
  'audio-console': 'audioConsole',
  'track-meta': 'trackMeta',
  'album-art': 'albumArt',
  'track-source': 'muted',
  'toggle-button': 'toggle',
  visualizer: 'visualizer',
  'transport-row': 'transport',
  'transport-track': 'transportTrack',
  bands: 'bands',
  band: 'band',
  'band-value': 'bandValue',
  'band-label': 'bandLabel',
  'master-row': 'master',
  'presets-panel': 'presets',
  'preset-list': 'presetList',
  'backend-status': 'backendStatus',
  'backend-status-label': 'backendLabel',
  'backend-status-value': 'backendValue',
  'backend-status-actions': 'backendActions',
  'backend-status-error': 'backendError',
  'async-notice': 'asyncNotice',
  'async-loading': 'asyncLoading',
  'async-success': 'asyncSuccess',
  'async-error': 'asyncError',
  'media-inspector-grid': 'mediaInspectorGrid',
  'media-path-input': 'mediaPathInput',
  'media-actions': 'mediaActions',
  'media-details': 'mediaDetails',
  'media-route': 'mediaRoute',
  'media-preview-panel': 'mediaPreviewPanel',
  'media-preview': 'mediaPreview',
  'toggle-all': 'todoToggleAll',
  'todo-list': 'todoList',
  'todo-checkbox': 'todoCheckbox',
  'todo-filters': 'todoFilters',
  'sr-only': 'srOnly',
  // Workspace shell (sidebar + content column).
  workspace: 'workspace',
  sidebar: 'sidebar',
  'sidebar-open': 'sidebarOpen',
  'sidebar-brand': 'sidebarBrand',
  'sidebar-search': 'sidebarSearch',
  'sidebar-search-kbd': 'sidebarSearchKbd',
  'sidebar-nav': 'sidebarNav',
  'sidebar-item': 'sidebarItem',
  'sidebar-item-glyph': 'sidebarItemGlyph',
  'sidebar-dot': 'sidebarDot',
  'sidebar-section': 'sidebarSection',
  'sidebar-footer': 'sidebarFooter',
  'main-column': 'mainColumn',
  'content-topbar': 'contentTopbar',
  'menu-button': 'menuButton',
  'theme-toggle': 'themeToggle',
  'theme-toggle-icon': 'themeToggleIcon',
  'theme-toggle-label': 'themeToggleLabel',
  'content-title': 'contentTitle',
  'content-body': 'contentBody',
  scrim: 'scrim',
  divider: 'divider',
  'sidebar-group': 'sidebarGroup',
  'home-page': 'homePage',
  'home-hero': 'homeHero',
  'home-hero-copy': 'homeHeroCopy',
  'home-title': 'homeTitle',
  'home-mark': 'homeMark',
  'home-mark-span': 'homeMarkSpan',
  'home-mark-small': 'homeMarkSmall',
  'home-toolbar': 'homeToolbar',
  'home-section-title': 'homeSectionTitle',
  'home-count': 'homeCount',
  'row-topline': 'rowTopline',
  'row-symbol': 'rowSymbol',
  'home-footer': 'homeFooter',
  'home-shortcut': 'homeShortcut',
  'palette-overlay': 'paletteOverlay',
  palette: 'palette',
  'palette-header': 'paletteHeader',
  'palette-title': 'paletteTitle',
  'palette-hint': 'paletteHint',
  'palette-results': 'paletteResults',
  'palette-item': 'paletteItem',
  'palette-glyph': 'paletteGlyph',
  'palette-copy': 'paletteCopy',
  'palette-copy-small': 'paletteCopySmall',
  'palette-key': 'paletteKey',
  'palette-empty': 'paletteEmpty',
  'palette-footer': 'paletteFooter',
  // Essay workspace (Chain Notes redesign).
  'essay-page': 'essayPage',
  'essay-layout': 'essayLayout',
  'essay-list-pane': 'essayListPane',
  'essay-list-scroll': 'essayListScroll',
  'essay-item': 'essayItem',
  'essay-chain-no': 'essayChainNo',
  'essay-editor-pane': 'essayEditorPane',
  'chain-nav': 'chainNav',
  'seg-group': 'segGroup',
  'seg-button': 'segButton',
  'essay-input': 'essayInput',
  'idea-input': 'ideaInput',
  'preview-article': 'previewArticle',
  'essay-frame': 'essayFrame',
  'map-frame': 'mapFrame',
  'frame-pane': 'framePane',
  'frame-divider': 'frameDivider',
  'frame-divider-wide': 'frameDividerWide',
  // Focused two-workspace launcher.
  'focus-shell': 'focusShell',
  'focus-launcher': 'focusLauncher',
  'focus-kicker': 'focusKicker',
  'focus-title': 'focusTitle',
  'focus-menu': 'focusMenu',
  'focus-choice': 'focusChoice',
  'focus-choice-eyebrow': 'focusChoiceEyebrow',
  'focus-choice-arrow': 'focusChoiceArrow',
  'focus-theme-toggle': 'focusThemeToggle',
  'focus-theme-toggle-inline': 'focusThemeToggleInline',
  'focus-error': 'focusError',
  'focus-panel': 'focusPanel',
  'focus-panel-header': 'focusPanelHeader',
  'focus-back': 'focusBack',
  'focus-panel-layout': 'focusPanelLayout',
  'focus-panel-intro': 'focusPanelIntro',
  'focus-side-panel': 'focusSidePanel',
  'focus-side-panel-heading': 'focusSidePanelHeading',
  'focus-feature-grid': 'focusFeatureGrid',
  'focus-feature': 'focusFeature',
  'focus-feature-symbol': 'focusFeatureSymbol',
  'feature-panel': 'featurePanel',
  'feature-panel-header': 'featurePanelHeader',
  'feature-panel-body': 'featurePanelBody',
  // Image gallery and lightbox.
  'gallery-grid': 'galleryGrid',
  'gallery-thumb': 'galleryThumb',
  'gallery-thumb-img': 'galleryThumbImg',
  'gallery-thumb-label': 'galleryThumbLabel',
  'lightbox-overlay': 'lightboxOverlay',
  'lightbox-toolbar': 'lightboxToolbar',
  'lightbox-toolbar-left': 'lightboxToolbarLeft',
  'lightbox-toolbar-right': 'lightboxToolbarRight',
  'lightbox-body': 'lightboxBody',
  'lightbox-image': 'lightboxImage',
  'lightbox-nav': 'lightboxNav',
  'lightbox-nav-prev': 'lightboxNavPrev',
  'lightbox-nav-next': 'lightboxNavNext',
  'lightbox-counter': 'lightboxCounter',
  'lightbox-close': 'lightboxClose',
  'lightbox-info': 'lightboxInfo'
};

export function sx(...classNames) {
  const values = classNames.flatMap((value) => {
    if (value && typeof value === 'object') return [value];
    return String(value || '')
      .split(/\s+/)
      .map((name) => {
        const camelName = name.replace(/-([a-z])/g, (_, letter) =>
          letter.toUpperCase()
        );
        return styles[aliases[name] || camelName];
      })
      .filter(Boolean);
  });
  return stylex.props(...values).className;
}

export function toneStyle(tone, variant = '') {
  if (tone === 'violet') tone = 'purple';
  if (variant === true) variant = 'mark';
  const suffix = variant ? variant[0].toUpperCase() + variant.slice(1) : '';
  const name = `${tone[0].toUpperCase()}${tone.slice(1)}${suffix}`;
  return styles[name] || styles.gold;
}
