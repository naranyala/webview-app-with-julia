...
```julia
using CooperativeJobManager

mgr = JobManager(max_jobs=128, retention_seconds=600.0)

# Create a job
id = create_job!(mgr; kind="download", metadata=Dict("url" => "https://example.com"))

# Worker loop
@async while true
    is_cancelled(mgr, id) && break
    # ... do work ...
    update_job!(mgr, id; progress=0.5, message="halfway")
end

# Complete
complete_job!(mgr, id; result=Dict("status" => "ok"))

# Query
snap = snapshot(mgr, id)
```
"""
module CooperativeJobManager
...
"""
    create_job!(manager; kind="job", metadata=nothing, correlation_id="") -> String
...
`correlation_id` links the job to an external request or operation.
"""
function create_job!(manager::JobManager; kind::AbstractString="job", metadata=nothing, correlation_id::AbstractString="")
